...
def generate_predictions_pipeline(
    ...
    ensambling_model = get_path(DEPLOYMENT_MODELS, 'RandomForest.sav')
    threshold = 0.49335053756436065
    ...
    try:
        ...
        model_ensambler = RandomForest()
        model_ensambler.load_model(ensambling_model)

        final_data = pd.merge(
            left=db.df,
            right=model_ensambler.predict(data, threshold, **predictions),
            on=['PROCESSED_IMG'],
            how='left'
        )
        ...
        signal_error.emit_error(__name__, info, err, True, True)
    else:
